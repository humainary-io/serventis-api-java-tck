...
final class OperationOutcomeMappingContractTest {
  ...
  private static Stream< Arguments > operationDomains() {
    ...
    return
      Stream.of(
        ...
        Arguments.of("Changes", Changes.Sign.class, Changes.KIND, Changes.OPERATION)
      );
  ...
  private static Stream< Arguments > outcomeDomains() {
    ...
    return
      Stream.of(
        ...
        Arguments.of("Changes", Changes.Sign.class, Changes.KIND, Changes.OUTCOME),
        Arguments.of("Guards", Guards.Sign.class, Guards.KIND, Guards.OUTCOME)
      );
  ...
  void changesPins() {
    ...
    assertEquals(BEGIN, Changes.OPERATION.apply(Changes.Sign.START));
    assertEquals(ADVANCE, Changes.OPERATION.apply(Changes.Sign.PROGRESS));
    assertEquals(END, Changes.OPERATION.apply(Changes.Sign.APPLY));
    assertEquals(END, Changes.OPERATION.apply(Changes.Sign.FAIL));
    assertEquals(END, Changes.OPERATION.apply(Changes.Sign.REJECT));
    assertEquals(END, Changes.OPERATION.apply(Changes.Sign.REVERT));

    assertEquals(SUCCESS, Changes.OUTCOME.apply(Changes.Sign.APPLY));
    assertEquals(FAIL, Changes.OUTCOME.apply(Changes.Sign.FAIL));
    assertEquals(FAIL, Changes.OUTCOME.apply(Changes.Sign.REJECT));
    assertEquals(FAIL, Changes.OUTCOME.apply(Changes.Sign.REVERT));
    assertNull(Changes.OUTCOME.apply(Changes.Sign.PROGRESS));
  ...
  void guardsPins() {
    ...
    assertEquals(SUCCESS, Guards.OUTCOME.apply(Guards.Sign.GRANT));
    assertEquals(FAIL, Guards.OUTCOME.apply(Guards.Sign.DENY));
    assertNull(Guards.OUTCOME.apply(Guards.Sign.ATTEMPT));
    assertNull(Guards.OUTCOME.apply(Guards.Sign.CHALLENGE));
  ...
  void registryIsComplete() {
    ...
    assertEquals(11, operationDomains().count());
    assertEquals(13, outcomeDomains().count());
