...
final class StatusMappingContractTest {
  ...
  private static Stream< Arguments > statusMaps() {
    ...
    return
      Stream.of(
        ...
        Arguments.of("Queues", Queues.Sign.class, Queues.STATUS),
        Arguments.of("Stacks", Stacks.Sign.class, Stacks.STATUS),
        Arguments.of("Pipelines", Pipelines.Sign.class, Pipelines.STATUS),
        Arguments.of("Messages", Messages.Sign.class, Messages.STATUS),
        Arguments.of("Members", Members.Sign.class, Members.STATUS),
        Arguments.of("Changes", Changes.Sign.class, Changes.STATUS),
        Arguments.of("Guards", Guards.Sign.class, Guards.STATUS)
      );
  ...
  void mappingsAreExact() {
    ...
    assertExact(
      Queues.Sign.class, Queues.STATUS,
      Map.of(
        Queues.Sign.ENQUEUE, STABLE,
        Queues.Sign.OVERFLOW, DEGRADED
      )
    );

    assertExact(
      Stacks.Sign.class, Stacks.STATUS,
      Map.of(
        Stacks.Sign.PUSH, STABLE,
        Stacks.Sign.OVERFLOW, DEGRADED
      )
    );

    assertExact(
      Pipelines.Sign.class, Pipelines.STATUS,
      Map.of(
        Pipelines.Sign.OUTPUT, STABLE,
        Pipelines.Sign.OVERFLOW, DEGRADED,
        Pipelines.Sign.LAG, DEGRADED
      )
    );

    assertExact(
      Messages.Sign.class, Messages.STATUS,
      Map.of(
        Messages.Sign.ACK, STABLE,
        Messages.Sign.NACK, DEGRADED,
        Messages.Sign.EXPIRE, DEGRADED,
        Messages.Sign.EXHAUST, DEFECTIVE
      )
    );

    assertExact(
      Members.Sign.class, Members.STATUS,
      Map.of(
        Members.Sign.REFUTE, STABLE,
        Members.Sign.ELECT, STABLE,
        Members.Sign.SUSPECT, DEGRADED
      )
    );

    assertExact(
      Changes.Sign.class, Changes.STATUS,
      Map.of(
        Changes.Sign.APPLY, STABLE,
        Changes.Sign.FAIL, DEGRADED,
        Changes.Sign.REVERT, DEGRADED
      )
    );

    assertExact(
      Guards.Sign.class, Guards.STATUS,
      Map.of(
        Guards.Sign.GRANT, STABLE,
        Guards.Sign.DENY, DEGRADED
      )
    );
